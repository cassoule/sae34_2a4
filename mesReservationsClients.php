...
foreach ($dbh->query("SELECT * from SAE._reservation join SAE._message on id_message_devis = id_message join SAE._compte on id_contient_proprietaire = _compte.id_compte") as $row) {
    foreach ($row as $key => $value) {
        if ($value == $idClient and $key == 'id_contient_client') {
            $cpt = $cpt + 1;
            $res[$cpt] = $row;
        }
    }
...
//annuler une reservation a finir erreur

if (isset($_POST['annulerReservation'])) {
    foreach ($dbh->query("SELECT * FROM _message_devis INNER JOIN _message ON _message_devis.id_message_devis = _message.id_message WHERE id_contient_client = $idClient AND id_logement = :id_logement") as $row) { // id_logement !!!!
        $stmtLogement->bindParam(':id_logement', $res[$key][0]['id_logement']);
...
<html lang="fr">
    ...
    <body>
        ...
        <?php if ($cpt >= 2) {
            echo "<main class='both-svg-main'>";
        } else {
            echo "<main class='bottom-svg-main'>";
        } ?>
        ...
        <div class="big-card-Resa">
            <div class="recherche">
                ...
                if ($cpt > 0) {
                    foreach ($res as $key => $value) {
                        //print_r($value['id_reservation']);
                        $id_res = $value['id_reservation'];
                ?>
                    <div class="big-card-Resa">
                        <div class="carroussel">
                            ...
                            <form action="/logementDetail.php?idLogement=<?php echo $value[0]['id_logement'] ?>" method="post">
                                <input type="submit" value="Détail logement">
                                <br>
                                <br>
                            </form>
                        </div>
    ...
    <script>
    function confirmAndExecute(res) {
        var confirmation = confirm("Êtes-vous sûr de vouloir supprimer votre reservation ?");
        if (confirmation) {
            window.location.href = "AnnulerResa.php/?id_reservation=" + res;
        } else {
            alert("Vous n'avez pas annuler");
        }
    }
</script>
</html>