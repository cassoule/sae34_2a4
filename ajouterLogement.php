...
$libelle = $accroche = $description = $max_personnes = $nature_logement = $type_logement = $surface = $nb_chambres = $nb_lits_simple = $nb_lits_double = $nb_salle_de_bain = $tarif_nuit_HT = $code_postal = $adresse = $complement_adresse = $ville = '';
...
try {
    if (($_SERVER['REQUEST_METHOD'] == 'POST') && (isset($_POST['libelle'])) && (isset($_POST['accroche'])) && (isset($_POST['description']))) {
        ...
        $tarif_nuit_HT = test_input($_POST['tarif_nuit_HT']);
        ...
        if ($max_personnes < 0 || $surface < 0 || $nb_chambres < 0 || $nb_lits_simple < 0 || $nb_lits_double < 0 || $nb_salle_de_bain < 0 || $tarif_nuit_HT < 0) {
            //echo "Erreur : Les champs de nombre ne peuvent pas être négatifs.  ";
            $hasErrors = true;
        }

        // Traiter des cas d'erreurs (à compléter)
        ...
        if (!$hasErrors) {
            ...
            if ($stmtAdresse->execute()) {
                ...
                if ($stmtLogement->execute()) {
                    ...
                    $stmtLogement = $dbh->prepare("SELECT id_logement FROM SAE._logement WHERE libelle_logement = :libelle_logement AND id_adresse = :id_adresse AND accroche = :accroche AND description_detaille = :description_detaille");
                    $stmtLogement->bindParam(':libelle_logement', $libelle, PDO::PARAM_STR);
                    $stmtLogement->bindParam(':accroche', $accroche, PDO::PARAM_STR);
                    $stmtLogement->bindParam(':description_detaille', $description, PDO::PARAM_STR);
                    $stmtLogement->bindParam(':id_adresse', $row['id_adresse'], PDO::PARAM_INT);
                    ...
                    foreach ($dbh->query("SELECT * FROM sae._logement ORDER BY id_logement DESC") as $newLogement) {
                        if (($newLogement['libelle_logement'] == $libelle) && ($newLogement['description_detaille'] == $description) && ($newLogement['accroche'] == $accroche) && ($newLogement['id_adresse'] == $newAdresse)) {
                            $id_logement = $newLogement['id_logement'];
                        }
                    }
            
                    // Renvoies vers la page des logements de l'utilisateur et arrête le script de la page
                    header("Location: https://site-sae-sixpetitscochons.bigpapoo.com/previsualiserLogement.php?id_logement=$id_logement");
                    exit();
                }
            }
        }
...
<html lang="fr">
    ...
    <body>
        ...
        <main class="both-svg-main-ajout">
            <h1 class="ajout-titre">Ajouter un Logement</h1>

            <!-- Formulaire d'enregistrement pour ajouter un logement -->
            <form class="enregistrement" action="/ajouterLogement.php" method="post" enctype="multipart/form-data">
                <div class="container-ajout">
                    <div class="container-ajout-detail cad-left">
                        <!-- Section pour les détails du logement -->
                        ...
                        <!-- Section pour les images du logement -->
                        <div class="ajout-card">
                            ...
                            <div id="image">
                                <div class="row">
                                    ...
                                    <input type="file" name="image1" accept=".jpg, .jpeg, .png" required>
                                </div>

                                <div class="row">
                                    <label for="image2">Image 2 :</label>
                                    <input type="file" name="image2" accept=".jpg, .jpeg, .png" required>
                                </div>

                                <div class="row">
                                    <label for="image3">Image 3 :</label>
                                    <input type="file" name="image3" accept=".jpg, .jpeg, .png" required>
                                </div>
                        ...
                        <!-- Section pour l'adresse du logement -->
                        <div class="ajout-card">
                            <div id="adresse">
                        ...
                        <!-- Section pour le prix du logement à la nuit -->
                        <div class="ajout-card">
                            <h2 class="ajout-mini-titre">Ajouter un tarif global par nuit et par personne</h2>
                            <div class="row">
                                <input type="number" name="tarif_nuit_HT" placeholder="Prix" required min="0" value="<?php echo $tarif_nuit_HT; ?>">
                            </div>
                        </div>
    ...
    <script>
        ...
        function dragOverHandler(event) {
            ...
            event.dataTransfer.dropEffect = "copy"; // Permettre le drop
        ...
        function dropHandler(event) {
            ...
            // Vérifier si l'élément déposé est une image
            if (event.dataTransfer.items) {
            // Utilisation de DataTransferItemList interface pour accéder au fichier
            for (var i = 0; i < event.dataTransfer.items.length; i++) {
                // Si les éléments déposés sont des fichiers, procéder
                ...
                if (file.type.match(/^image\//)) {
                    ...
                    alert("Vous ne pouvez pas ajouter plus de 7 images.");
                    }
                } else {
        ...
        function displayImage(file) {
            ...
            if (images.length < 4) {
                document.getElementById("drop-zone1").appendChild(container);
            } else {
                document.getElementById("drop-zone2").appendChild(container);
            }
            images.push(file);
            };
