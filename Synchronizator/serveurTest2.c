...
#define MAX_BUFFER_SIZE 4112
#define MAX_STRING_LOGS 4112
        ...
        while (1) {
            ...
            if (strcmp(buffer, "1") == 0) {
                ...
                if (privilegeUn == 1) {
                    ...
                    strcat(resultBuffer, "\nid_logement   libelle_logement   accroche   description_detaille   max_personnes   nature_logement   type_logement   surface   nb_chambres   nb_lits_simples   nb_lits_doubles   nb_salle_de_bain   tarif_nuit_ht   avis_logement_total   est_actif   id_proprietaire   id_adresse\n    | id_adresse   code_postal   adresse   complement_adresse   ville\n    | nom_equipement\n    | nom_amenagement\n    | nom_installation\n    | id_charge   nom_charge   prix_charge_ht   id_logement\n    | id_service   nom_service   prix_service_ht   id_logement\n\n");

                    // Prépare la requête permettant de récupérer tous les logements
                    snprintf(query, sizeof(query), "SELECT * FROM sae._logement WHERE id_proprietaire = '%s'", id_compte);
                    ...
                    for (i = 0; i < numRows; i++) {
                        ...
                        char complement_logement[8192]; // String stockant les requetes faites pour la base de données
                        PGresult *resultat_logement; // Stock le resultat des requetes
                        int f, g, rows, cols;

                        // Section pour récupérer les informations sur l'adresse du logement
                            // Prépare la requete permettant de recupérer l'adresse du logement
                            snprintf(complement_logement, sizeof(complement_logement), "SELECT * FROM sae._adresse WHERE id_adresse = '%s'", PQgetvalue(res, i, 16));

                            // Lance la requete et stock sont resultat dans la variable res 
                            resultat_logement = PQexec(connexion, complement_logement);

                            // Vérification de l'exécution de la requête dans le cas ou elle ne fonctionne pas
                            if (PQresultStatus(resultat_logement) != PGRES_TUPLES_OK) {
                                fprintf(stderr, "Échec de l'exécution de la requête : %s", PQerrorMessage(connexion));
                                PQclear(resultat_logement);
                                PQfinish(connexion);
                                exit(1);
                            }

                            rows = PQntuples(resultat_logement);
                            cols = PQnfields(resultat_logement);

                            // Parcours de resultat_logement
                            for (f = 0; f < rows; f++) {
                                for (g = 0; g < cols; g++) {
                                    // Ajouter chaque attribut suivi d'un séparateur à la chaîne résultante
                                    strcat(resultBuffer, PQgetvalue(resultat_logement, f, g));
                                    strcat(resultBuffer, "   ");
                                }
                            }
                            strcat(resultBuffer, "\n    | ");
                        
                        // Section pour les équipements du logement
                            // Prépare la requete permettant de recupérer les equipements du logement
                            snprintf(complement_logement, sizeof(complement_logement), "SELECT nom_equipement FROM sae._equipe WHERE id_logement = '%s'", PQgetvalue(res, i, 0));
                        ...
                        // Section pour les aménagements du logement
                            // Prépare la requete permettant de recupérer les equipements du logement
                            snprintf(complement_logement, sizeof(complement_logement), "SELECT nom_amenagement FROM sae._contient WHERE id_logement = '%s'", PQgetvalue(res, i, 0));
                        ...
                        // Section pour les installations du logement
                            // Prépare la requete permettant de recupérer les equipements du logement
                            snprintf(complement_logement, sizeof(complement_logement), "SELECT nom_installation FROM sae._possede WHERE id_logement = '%s'", PQgetvalue(res, i, 0));
                        ...
                        // Section pour les charges du logement
                            // Prépare la requete permettant de recupérer les equipements du logement
                            snprintf(complement_logement, sizeof(complement_logement), "SELECT * FROM sae._charge WHERE id_logement = '%s'", PQgetvalue(res, i, 0));
                        ...
                        // Section pour les services du logement
                            // Prépare la requete permettant de recupérer les equipements du logement
                            snprintf(complement_logement, sizeof(complement_logement), "SELECT * FROM sae._service WHERE id_logement = '%s'", PQgetvalue(res, i, 0));
                    ...
                    strcat(resultBuffer, "1) Afficher tous les biens d'un client (si vous etes avez les privileges)\n2) Afficher vos biens ou ceux d'une autre personne (si vous avez les privileges)\n3) Afficher le calendrier de disponibilite d'un bien sur une periode\n4) Mettre en indisponibilite un bien sur une periode\n5) Arreter la connexion avec le serveur\n\nChoisissez votre action : ");
            ...
            // Si le client a taper 2 pour afficher tous les logements
                ...
                if (privilegeDeux == 1) {
                    ...
                    // Prépare la requête permettant de récupérer tous les logements
                    snprintf(query, sizeof(query), "SELECT * FROM sae._logement");
